...
export function QuickAddDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  useEffect(() => {
    if (open) { setAmount("0"); setDescription(""); setDebtId(""); }
  }, [open]);
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl bg-secondary/60 p-4 text-center">
            ...
            <div className="mt-1 text-xs text-muted-foreground">{TYPES.find((t) => t.v === type)?.label}</div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <Select value={category} onValueChange={setCategory}>
                ...
                <SelectContent>{budget.map((b) => <SelectItem key={b.id} value={b.name}>{b.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div>
            ...
            {type === "debt_payment" && (
              <div className="col-span-2">
                ...
                <Select value={debtId} onValueChange={setDebtId}>
                  <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent>{debts.map((d) => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            )}
            <div className="col-span-2">
              <Label className="text-xs">Description</Label>
              <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Merchant / note" />
            </div>
            {type === "expense" && (
              <div className="col-span-2 flex gap-2">
                <Button variant={needWant === "need" ? "default" : "outline"} className="flex-1" onClick={() => setNeedWant("need")}>Need</Button>
                <Button variant={needWant === "want" ? "default" : "outline"} className="flex-1" onClick={() => setNeedWant("want")}>Want</Button>
              </div>
            )}
            <div className="col-span-2 flex gap-2">
              <Button variant={cleared ? "default" : "outline"} className="flex-1" onClick={() => setCleared(true)}>Cleared</Button>
              <Button variant={!cleared ? "default" : "outline"} className="flex-1" onClick={() => setCleared(false)}>Pending</Button>
            </div>
          </div>
          <Button className="w-full" onClick={save} disabled={saving}>{saving ? "Saving…" : "Save transaction"}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
