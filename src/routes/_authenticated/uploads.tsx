...
export const Route = createFileRoute("/_authenticated/uploads")({
  head: () => ({
    meta: [
      { title: "Statement Uploads — Wealthpilot" },
      {
        name: "description",
        content:
          "Upload bank, credit card, pay stub, and retirement statements and review every extracted item before saving.",
      },
    ],
  }),
    ...
    <div className="space-y-5 pb-8">
      <header>
        ...
        <p className="text-xs text-muted-foreground">
          CSV, TXT, and text-based PDF. Nothing is saved until you review it.
        </p>
      </header>
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-border bg-card p-4 text-xs text-muted-foreground">
        ...
        <span>
          Everything extracted lands in a review table first. Wealthpilot never updates a balance or
          adds a transaction without your confirmation, and flags anything that looks like a
          duplicate.
        </span>
      </div>
        ...
        {!isLoading && !imports.length && (
          <div className="rounded-2xl border border-dashed border-border bg-card p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              No statements imported yet. Upload one above to get started.
            </p>
          </div>
        )}
        {imports.map((im) => (
          <button
            ...
            className="flex w-full items-center gap-3 rounded-2xl border border-border bg-card p-4 text-left hover:bg-secondary/40"
          >
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-secondary text-accent">
              <FileText className="h-4 w-4" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm">{im.file_name}</div>
              <div className="text-xs text-muted-foreground">
                {KINDS.find((k) => k.value === im.doc_kind)?.label ?? im.doc_kind} ·{" "}
                {im.items_total} item{im.items_total === 1 ? "" : "s"}
                {im.statement_end ? ` · to ${dateShort(im.statement_end)}` : ""}
              </div>
            </div>
            <span
              className={`rounded-full px-2 py-0.5 text-[10px] uppercase tracking-wider ${im.status === "applied" ? "bg-success/20 text-success" : im.status === "discarded" ? "bg-secondary text-muted-foreground" : "bg-warning/20 text-warning"}`}
            >
              {im.status === "applied" ? `${im.items_accepted} saved` : im.status}
            </span>
          </button>
        ))}
      </section>
  ...
  async function handleFile(file: File) {
    ...
    try {
      if (file.size > 15 * 1024 * 1024)
        throw new Error("That file is larger than 15 MB. Please split it or export a CSV.");
      ...
      if (parsed.items.length) {
        const rows = parsed.items.map((it) => {
          const dup = findDuplicate(
            { occurred_on: it.occurred_on, amount: it.amount, description: it.description },
            existingTx.map((t) => ({
              id: t.id,
      ...
      await logAudit(
        ...
        "statement_import",
        "created",
        `Uploaded ${file.name} (${parsed.items.length} candidate items)`,
        { doc_kind: parsed.docKind },
        imp.id,
      );
      if (parsed.warning) toast.warning(parsed.warning);
        ...
        toast.success(
          `${parsed.items.length} item${parsed.items.length === 1 ? "" : "s"} ready for review`,
        );
      onDone(imp.id);
    } catch (e) {
    ...
    <section className="rounded-3xl border border-dashed border-border bg-card p-6">
      ...
      <div className="mt-5 text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          {busy ? "Reading your statement…" : "CSV, TSV, TXT, or text-based PDF · up to 15 MB"}
        </p>
        <input
          ref={inputRef}
          type="file"
...
function ReviewScreen({ importId, onBack }: { importId: string; onBack: () => void }) {
  ...
  async function apply() {
    ...
    try {
      ...
      await supabase
        .from("statement_imports")
        .update({ status: "applied", items_accepted: rows.length })
        .eq("id", importId);
      await logAudit(
        user.id,
        "statement_import",
        "applied",
        `Saved ${rows.length} transaction${rows.length === 1 ? "" : "s"} from ${im.file_name}`,
        { import_id: importId },
        importId,
      );
  ...
  async function applyDetected(key: string, value: number) {
    ...
    if (key === "minimum_payment" && im.debt_id) {
      await supabase
        .from("debts")
        .update({ minimum_payment: value, minimum_estimated: false })
        .eq("id", im.debt_id);
      await logAudit(
        user.id,
        "debt",
        "updated",
        `Minimum payment set to ${money(value)} from ${im.file_name}`,
        { field: "minimum_payment", value },
        im.debt_id,
      );
    ...
    if ((key === "new_balance" || key === "account_balance") && im.debt_id) {
      ...
      await logAudit(
        ...
        "debt",
        "updated",
        `${prev?.name ?? "Debt"} balance ${money(prev?.balance ?? 0)} → ${money(value)} from ${im.file_name}`,
        { field: "balance", from: prev?.balance, to: value },
        im.debt_id,
      );
      qc.invalidateQueries({ queryKey: ["debts", user.id] });
    ...
    if ((key === "account_balance" || key === "plan_value") && im.account_id) {
      ...
      await logAudit(
        ...
        "account",
        "updated",
        `${prev?.name ?? "Account"} balance ${money(prev?.balance ?? 0)} → ${money(value)} from ${im.file_name}`,
        { from: prev?.balance, to: value },
        im.account_id,
      );
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <header>
        ...
        <p className="text-xs text-muted-foreground">
          {im?.file_name} · {im?.statement_start ? `${dateShort(im.statement_start)} – ` : ""}
          {im?.statement_end ? dateShort(im.statement_end) : "period not detected"}
        </p>
      </header>
      ...
      {im && Object.keys(im.detected ?? {}).length > 0 && (
        <section className="rounded-2xl border border-border bg-card p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Apply each one only if it's correct. Nothing here is applied automatically.
          </p>
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {v.confidence < 0.7 && (
                      <span className="rounded-full bg-warning/20 px-2 py-0.5 text-[10px] uppercase text-warning">
                        low confidence
                      </span>
                    )}
                  </div>
                ...
                <div className="tabular-nums">
                  {typeof v.value === "number"
                    ? k.includes("apr") || k.includes("rate")
                      ? `${v.value}%`
                      : money(v.value)
                    : dateShort(String(v.value))}
                </div>
                {typeof v.value === "number" && !k.includes("apr") && !k.includes("rate") && (
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => applyDetected(k, v.value as number)}
                  >
                    Apply
                  </Button>
                )}
      ...
      <section className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="secondary" onClick={() => setAllDecision("accept")}>
              Accept all
            </Button>
            <Button size="sm" variant="secondary" onClick={() => setAllDecision("exclude")}>
              Exclude all
            </Button>
          </div>
        </div>
        ...
        {!isLoading && !items.length && (
          <div className="rounded-2xl border border-dashed border-border bg-card p-6 text-center text-sm text-muted-foreground">
            No transaction lines were recognised in this file. You can still apply the detected
            values above, or add entries manually.
          </div>
        )}
        {items.map((it) => (
          <div
            key={it.id}
            className={`rounded-2xl border p-4 ${it.decision === "exclude" ? "border-border bg-card opacity-60" : it.duplicate_of ? "border-warning/50 bg-card" : "border-border bg-card"}`}
          >
            <div className="flex flex-wrap items-center gap-2">
              <Input
                ...
                onChange={(e) => patch(it.id, { occurred_on: e.target.value })}
              />
              <Input
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              <Input
                className="h-9 w-40 text-base md:h-8 md:text-xs"
              ...
              <div className="ml-auto flex gap-1">
                {(["accept", "review", "exclude"] as const).map((d) => (
                  <button
                    key={d}
                    onClick={() => patch(it.id, { decision: d })}
                    className={`rounded-full px-3 py-1 text-xs capitalize ${it.decision === d ? (d === "accept" ? "bg-success/20 text-success" : d === "exclude" ? "bg-destructive/20 text-destructive" : "bg-warning/20 text-warning") : "bg-secondary text-muted-foreground"}`}
                  >
                    {d}
                  </button>
                ))}
      ...
      <div className="sticky bottom-24 z-20 flex gap-2 rounded-2xl border border-border bg-card/95 p-3 backdrop-blur lg:bottom-4">
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                The uploaded statement record and all {items.length} candidate items are removed. No
                saved transactions or balances change.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Save {accepted.length} transactions?</AlertDialogTitle>
              <AlertDialogDescription>
                Only items marked “accept” are added. Excluded and review items stay unsaved, and no
                balances change unless you applied a detected value.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction onClick={apply}>Save them</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
