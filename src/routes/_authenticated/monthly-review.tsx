...
export const Route = createFileRoute("/_authenticated/monthly-review")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A guided monthly financial review using your saved Wealthpilot data.",
      },
    ],
  }),
...
function MonthlyReviewPage() {
  ...
  useEffect(() => {
    ...
    async function load(uid: string) {
      ...
      const [a, d, g, r] = await Promise.all([
        ...
        supabase
          .from("debts")
          .select("id,name,balance,pending,apr,credit_limit")
          .eq("user_id", uid),
        supabase.from("goals").select("id,name,current_amount,target_amount").eq("user_id", uid),
        supabase
          .from("recurring_transactions")
          .select("id,name,amount,kind,is_active")
          .eq("user_id", uid)
          .eq("is_active", true),
      ]);
      if (!cancelled) {
  ...
  const summary = useMemo(() => {
    const assets = accounts
      .filter((a) => ASSET_KINDS.has(a.kind))
    ...
    const bills = recurring
      .filter((r) => r.kind !== "income" && r.kind !== "subscription")
      .reduce((sum, r) => sum + Number(r.amount || 0), 0);
  ...
  const tasks = [
    {
      ...
      detail: "Confirm chequing, savings, investments, and retirement balances are current.",
      to: "/net-worth",
    },
    {
      id: "cards",
      title: "Verify card payments",
      detail: "Confirm recent payments posted and pending amounts are accurate.",
      to: "/debts",
    },
    {
      id: "bills",
      title: "Review upcoming bills",
      detail: "Check due dates, auto-pay status, and cash required before payday.",
      to: "/bill-calendar",
    },
    {
      id: "subscriptions",
      title: "Review subscriptions",
      detail: "Cancel, pause, or question anything that no longer provides value.",
      to: "/subscriptions",
    },
    {
      id: "goals",
      title: "Update goal progress",
      detail: "Record contributions and confirm your next monthly target.",
      to: "/goals",
    },
    {
      id: "forecast",
      title: "Check next 30 days",
      detail: "Review your lowest projected balance before optional spending.",
      to: "/cash-flow",
    },
  ];
    ...
    <div className="space-y-4 pb-8">
      <header className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            A focused check-in to keep your data and decisions on track.
          </p>
        </div>
      </header>
      ...
      <section className="rounded-3xl bg-violet-grad p-5 text-primary-foreground shadow-card">
        ...
        <div className="mt-2 flex items-end justify-between gap-3">
          ...
          <div className="text-sm opacity-80">
            {completed === tasks.length ? "Complete" : "Items checked"}
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="mt-3 space-y-2 text-sm text-muted-foreground">
          <p>
            {summary.highestApr
              ? `Highest-cost debt: ${summary.highestApr.name} at ${Number(summary.highestApr.apr || 0).toFixed(2)}% APR.`
              : "Add debt APRs to identify your highest-cost balance."}
          </p>
          <p>
            {summary.closestGoal
              ? `Closest goal: ${summary.closestGoal.name} at ${Math.min(100, Math.round((Number(summary.closestGoal.current_amount || 0) / Math.max(1, Number(summary.closestGoal.target_amount || 0))) * 100))}% complete.`
              : "Add a financial goal to track monthly progress."}
          </p>
          <p>
            Subscriptions currently represent about {money(summary.subscriptions)} per billing cycle
            based on saved recurring items.
          </p>
        </div>
      </section>
        ...
        {tasks.map((task) => (
          <div key={task.id} className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-start gap-3">
              ...
              >
                <CheckCircle2
                  className={`h-5 w-5 ${checked[task.id] ? "text-success" : "text-muted-foreground"}`}
                />
              </button>
              ...
              <Link to={task.to} className="shrink-0 text-accent">
      ...
      <p className="text-center text-[10px] text-muted-foreground">
        This review is read-only. Checklist progress is stored privately on this device.
      </p>
    </div>
  );
}
