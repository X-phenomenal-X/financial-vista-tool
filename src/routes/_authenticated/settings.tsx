...
export const Route = createFileRoute("/_authenticated/settings")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Profile, secure backups, exports, restore, audit history, and privacy.",
      },
    ],
  }),
...
function SettingsPage() {
  ...
  async function restoreFile(file: File) {
    ...
    try {
      ...
      if (
        !window.confirm(
          "Restore missing records from this backup? Existing record IDs will be skipped and nothing will be overwritten.",
        )
      ) {
        return;
      }
      ...
      setRestoreSummary(
        `${inserted} inserted · ${skipped} duplicates skipped${errors.length ? ` · ${errors.length} table errors` : ""}`,
      );
      ...
      if (errors.length)
        toast.warning("Restore finished with some table errors. Review the summary and history.");
  ...
  const filteredAudit = audit
    .filter((item) => auditEntity === "all" || item.entity === auditEntity)
    .slice(0, 40);
    ...
    <div className="space-y-5 pb-8">
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <div className="mt-3 space-y-1 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span>{profile?.is_owner ? "Yes" : "No"}</span>
          </div>
          <div className="flex justify-between">
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Install Wealthpilot to your home screen for full-screen use. Recently viewed screens stay
          readable without a connection; edits need you back online.
        </p>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <span
            className={`rounded-full px-3 py-1 text-xs ${pwa.online ? "bg-success/15 text-success" : "bg-warning/15 text-warning"}`}
          >
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          The JSON backup includes accounts, debts, budgets, transactions, goals, reminders, payday
          plans, and statement-import review data.
        </p>
        ...
        <div className="mt-3 rounded-xl bg-secondary/40 px-3 py-3 text-xs text-muted-foreground">
          Restore never overwrites an existing record ID. New records require confirmation and
          duplicate IDs are skipped.
        </div>
        {restoreSummary && (
          <div className="mt-2 rounded-xl bg-success/10 px-3 py-2 text-xs text-success">
            {restoreSummary}
          </div>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Export individual sections for spreadsheets or your own records.
        </p>
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
          <ExportButton
            label="Transactions"
            onClick={() => exportCsv("transactions", ctx.transactions)}
          />
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Important imports, balance edits, plans, reminders, and backups.
              </p>
            </div>
          </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          Wealthpilot is private and single-owner. Row-level security scopes financial records to
          the signed-in account. Uploaded documents require review and confirmation before any
          balance or transaction is saved.
        </p>
      </section>
      ...
      <section className="rounded-2xl bg-hero p-5 shadow-card">
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          The coach reads live app values and works without a paid AI API. A future optional AI
          provider can be connected without replacing these calculations.
        </p>
      </section>
