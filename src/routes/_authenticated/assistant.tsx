...
export const Route = createFileRoute("/_authenticated/assistant")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A private financial copilot using your live Wealthpilot data.",
      },
    ],
  }),
...
function AssistantPage() {
  ...
  const activeSources = [
    ...
    { label: "Payday plans", value: paydayPlans.length },
    {
      ...
      value: statementImports.filter((item) => item.status === "pending").length,
    },
  ];
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-8rem)] flex-col pb-4">
      ...
      <header className="relative z-10 mb-4 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full border border-accent/20 bg-accent/10 px-2 py-0.5 text-[9px] font-medium uppercase tracking-[0.16em] text-accent">
              Live
            </span>
          </div>
        ...
        <div className="relative z-20 mb-4 flex items-center gap-3 rounded-2xl border border-destructive/20 bg-destructive/10 p-3 text-sm shadow-card animate-in fade-in slide-in-from-top-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-muted-foreground">
              Your saved chat history on this device will be cleared.
            </div>
          </div>
          <button
            onClick={() => setShowClearConfirm(false)}
            className="grid h-8 w-8 place-items-center rounded-lg hover:bg-card/60"
            aria-label="Cancel"
          >
            <X className="h-4 w-4" />
          </button>
          <button
            onClick={clearChat}
            className="rounded-xl bg-destructive px-3 py-2 text-xs font-medium text-destructive-foreground transition active:scale-95"
          >
            Clear
          </button>
        </div>
      ...
      <section className="relative z-10 mb-4 overflow-hidden rounded-[1.75rem] border border-accent/20 bg-gradient-to-br from-card via-card to-accent/10 p-5 shadow-elevated">
        ...
        <div className="relative flex items-start gap-4">
          ...
          <Link
            ...
            style={{
              background: `conic-gradient(hsl(var(--accent)) ${health.score * 3.6}deg, hsl(var(--secondary)) 0deg)`,
            }}
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-muted-foreground">
                {totalSources} records across {activeSources.length} sources
              </div>
            </div>
          </div>
          <ChevronDown
            className={`h-4 w-4 text-muted-foreground transition-transform ${showSources ? "rotate-180" : ""}`}
          />
        </button>
        ...
        {showSources && (
          <div className="relative mt-2 grid grid-cols-2 gap-2 rounded-2xl border border-border/60 bg-background/35 p-3 animate-in fade-in slide-in-from-top-2">
            {activeSources.map((source) => (
              <div
                key={source.label}
                className="flex items-center gap-2 rounded-xl bg-card/70 p-2.5"
              >
      ...
      <div className="relative z-10 mb-4 grid grid-cols-3 gap-2">
        <Link
          to="/bill-calendar"
          className="group rounded-2xl border border-border bg-card p-3 text-center shadow-card transition duration-200 hover:-translate-y-0.5 hover:border-accent/40 active:scale-[0.97]"
        >
      ...
      <div className="relative z-10 mb-2 flex items-center justify-between">
        <div>
          ...
          <div className="mt-0.5 text-[10px] text-muted-foreground">
            Answers use your current Wealthpilot records
          </div>
        </div>
      ...
      <div className="relative z-10 flex-1 space-y-4 overflow-y-auto pb-2" aria-live="polite">
        {msgs.map((message, index) => (
          <div
            ...
            className={
              message.role === "user"
                ? "flex justify-end animate-in fade-in slide-in-from-bottom-2"
          ...
          <div className="min-w-0 flex-1 px-2 py-1">
            <textarea
              ...
              placeholder={
                loading
                  ? "Reading your data…"
                  : "Ask about spending, debt, bills, goals, or payday…"
              }
              disabled={loading}
            ...
            <div className="flex items-center justify-between gap-2 text-[9px] text-muted-foreground">
              <span className="flex items-center gap-1">
                <ShieldCheck className="h-3 w-3 text-accent" /> Uses saved app data
              </span>
      ...
      <p className="relative z-10 mt-2 px-3 text-center text-[9px] leading-relaxed text-muted-foreground">
        Wealthpilot provides calculations and rules from saved app data, not personalized investment
        or credit-product advice.
      </p>
    </div>
  );
}
