...
export function financialHealth(ctx: CoachContext): FinancialHealthResult {
  ...
  const factors: HealthFactor[] = [
    {
      ...
      score: cashScore,
      max: 25,
      detail: cash >= CASH_BUFFER ? `You have at least the ${CASH_BUFFER.toLocaleString("en-CA", { style: "currency", currency: "CAD" })} safety buffer.` : "Your available cash is below the safety buffer.",
    ...
    },
    {
      ...
      max: 25,
      detail: activeCards.length ? `Average utilization is ${Math.round(avgUtil * 100)}%.` : "No revolving credit balances are being tracked.",
    ...
    },
    {
      ...
      score: paymentScore,
      max: 20,
      detail: overdue ? `${overdue} debt item${overdue === 1 ? " is" : "s are"} past due.` : "No tracked debt is marked past due.",
    ...
    },
    {
      ...
      score: debtScore,
      max: 20,
      detail: cardDebt ? `${cardDebt.toLocaleString("en-CA", { style: "currency", currency: "CAD" })} in high-interest debt is being tracked.` : "No high-interest debt is being tracked.",
    ...
    },
    {
      ...
      score: goalScore,
      max: 10,
      detail: goals.length ? `${Math.round(goalProgress * 100)}% average progress across active goals.` : "Add a goal to start building this part of the score.",
  ...
  const nextBestAction = weakest.label === "Cash buffer"
