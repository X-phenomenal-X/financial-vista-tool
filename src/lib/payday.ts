...
export function buildDefaultLines(
  debts: Debt[],
  budgetBills: { name: string; planned: number }[],
): PlanLine[] {
  ...
  active
    ...
    .forEach((d) =>
      lines.push({
        key: `overdue-${d.id}`,
        label: `${d.name} — PAST DUE minimum`,
        kind: "minimum",
        amount: Number(d.minimum_payment) || 0,
        debtId: d.id,
        locked: true,
        note: "Overdue — pay first",
      }),
    );
  // Bills before next payday.
  budgetBills.forEach((b, i) =>
    lines.push({
      key: `bill-${i}-${b.name}`,
      label: b.name,
      kind: "bill",
  ...
  priorityOrder({
    ...
    .forEach((d) =>
      lines.push({
        key: `min-${d.id}`,
        label: `${d.name} minimum`,
        kind: "minimum",
        amount: Number(d.minimum_payment) || 0,
        debtId: d.id,
        note: `${Number(d.apr).toFixed(2)}% APR`,
      }),
    );
...
export function avalancheTarget(debts: Debt[]) {
  return (
    priorityOrder({
      ...
      reminders: [],
      goals: [],
    }).filter((d) => d.kind !== "car_loan")[0] ?? null
  );
}
...
export function computeRunning(inp: PlannerInputs, extraLabel: string) {
  ...
  const all: { label: string; amount: number; kind: PlanLine["kind"] }[] = [
    ...
    ...(inp.spendingAllowance > 0
      ? [{ label: "Spending allowance", amount: inp.spendingAllowance, kind: "allowance" as const }]
      : []),
    ...(inp.extraDebtPayment > 0
      ? [{ label: extraLabel, amount: inp.extraDebtPayment, kind: "avalanche" as const }]
      : []),
  ];
  for (const a of all) {
    running -= a.amount;
    rows.push({
      ...
      running,
      kind: a.kind,
      breaksBuffer: running < inp.buffer,
    });
  }
