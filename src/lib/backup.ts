...
export async function restoreBackup(json: unknown, userId: string): Promise<RestoreReport[]> {
  ...
  if (!parsed || typeof parsed !== "object" || !parsed.tables)
    throw new Error("This file isn't a Wealthpilot backup.");
  ...
  for (const t of BACKUP_TABLES) {
    ...
    const fresh = rows
      .filter((r) => r.id && !existing.has(String(r.id)))
      .map((r) => ({ ...r, user_id: userId }));
    if (!fresh.length) {
      reports.push({ table: t, inserted: 0, skipped: rows.length });
      continue;
    }
    const { error } = await supabase.from(t).insert(fresh as never);
    reports.push({
      table: t,
      inserted: error ? 0 : fresh.length,
